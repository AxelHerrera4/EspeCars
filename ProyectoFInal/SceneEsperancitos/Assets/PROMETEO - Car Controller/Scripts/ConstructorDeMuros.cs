...
{
    public Transform rutaPadre; // Arrastra aqu� tu "Ruta_IA"
    public float alturaMuro = 3f; // Qu� tan altos quieres los muros
    public float anchoMuro = 1f;  // Qu� tan gordos (para que no los atraviesen)
    ...
    void Start()
    {
        ...
        for (int i = 0; i < rutaPadre.childCount; i++)
        {
            ...
            // El % hace que el �ltimo punto se una con el primero (cerrar circuito)
            ...
            // Lo ponemos hijo de este objeto para no ensuciar la jerarqu�a
            muro.transform.parent = this.transform;

            // 3. Posici�n: Justo en el centro entre los dos puntos
            muro.transform.position = (puntoA + puntoB) / 2;

            // 4. Rotaci�n: Que mire hacia el siguiente punto
            ...
            // pero dejamos el Collider (la f�sica)
